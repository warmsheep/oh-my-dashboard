...
export interface RewriteOptions {
  ...
  cspSource: string;
  /** Per-load nonce stamped onto every rewritten script tag. */
  nonce: string;
...
export function rewriteWebviewAssets(html: string, opts: RewriteOptions): string {
  const csp = `default-src 'none'; style-src ${opts.cspSource} 'unsafe-inline'; script-src ${opts.cspSource} 'nonce-${opts.nonce}';`;
  let out = html.replace(
    /(<script[^>]*?\ssrc=["'])([^"']+)(["'])/gi,
    (_match, before: string, src: string, after: string) =>
      `${before}${opts.assetUri(path.basename(src))}${after} nonce="${opts.nonce}"`,
  );
  out = out.replace(
