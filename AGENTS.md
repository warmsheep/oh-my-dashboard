...
- Coding Plan 额度（`src/core/quotaService.ts`）：从 `~/.local/share/opencode/auth.json` 读 Kimi/GLM/DeepSeek 凭据查官方接口（DeepSeek 按量计费只有余额无窗口、多币种取 CNY 优先）；MiMo 走 `platform.xiaomimimo.com` Dashboard Cookie（存 `<configDir>/quota.json`，只有月度窗口，无 5h/周）。接口地址与 GLM unit 枚举等细节见 quotaService.ts 注释。**线程池纪律（防拖垮宿主）**：一切请求（fetchAll/fetchProvider）经 `RequestGate` 并发闸门（默认同时在途 ≤2）——DNS 黑洞时 undici 的 getaddrinfo 占 libuv 线程池（exthost 默认 4 线程、全扩展共享 async fs），AbortSignal 取消不了已排队的查询；自动刷新在连续 `QUOTA_PAUSE_AFTER_STREAK`(=3) 轮传输级失败后熔断停摆（手动刷新成功/配置变更复位），激活首刷延迟 5s 避开启动 IO 高峰；额度与模型清单请求默认走 c-ares 版 `resilientFetch`（`src/core/resilientFetch.ts`，undici Agent + `dns.promises.Resolver` 自定义 lookup，DNS 查询走事件循环、正结果带 30s TTL 缓存、失败才回退 getaddrinfo），DNS 黑洞不再驻留 libuv 线程池线程；管理面板打开后超过 20s 仍未完成首次初始化（僵尸 iframe，探活只覆盖已就绪页面）时，下一次入口点击直接重建面板。**恢复路径**：窗口聚焦（onDidChangeWindowState）时若额度降级（任一已配置供应商出错或无快照）立即刷一轮（10s 节流、仅降级时触发）；刷新失败经 `spliceStaleProviders` 做 stale-while-error 兜底——30 分钟内（`STALE_PROVIDER_MAX_AGE_MS`）沿用该供应商最近成功数据（`staleFetchedAt` 标记，状态栏/面板渲染 `~` 前缀），错误字段保留故熔断判定不受影响。展示入口是 Webview 管理面板（`src/webview/managerPanelHost.ts` 单例 + `webview-ui/manager.html`「OMO/OpenCode/额度/设置/模板/技能」六选项卡单入口〔OMO 页（tab id 仍为 config，仅改显示标签）=模型配置即时编辑（configSetModel→setAgentModel）+ 功能设置区（OMO_MISC_SETTINGS 描述符驱动，omoSetSetting 即时写，乐观更新+失败回滚）；OpenCode 页=opencode.json 常用键可视化编辑（OPENCODE_SETTINGS 描述符驱动，opencodeSetSetting 即时写，「未设置」=删键）；技能页=Skills 只读清单（skillSummaries 读 SKILL.md frontmatter description，数据仍随 configInit 推送，无独立通道）〕，额度页按供应商分组、组内单独刷新（core `fetchProvider` + `mergeProviderSnapshot`）、MiMo Cookie 在分组内编辑；设置页显式保存链/回声抑制同宿主内；模板编辑为**单会话**——`presetEditorHost.ts` 是纯会话控制器（begin/save/noteDirty/cancel，无面板管理），editPreset 命令经 `openPresetEditorTab` 导航到模板选项卡，未保存草稿按模板名分槽存于 webview state〔`presetDrafts`〕、宿主 workspaceState 快照语义不变，「取消」清空会话不关面板）；额度数据形状与展示辅助函数收口在 `src/shared/protocol.ts`（core re-export）。**状态栏可见性开关**：quota.json 的 `statusBar` 块按供应商存布尔（缺省=显示，读路径容错非严格 false 一律显示）；隐藏供应商不参与定时刷新（`fetchAll(providerIds)` 子集轮次，熔断/降级判定只看本轮实际抓取的切片；管理面板可见〔onDidChangeViewState〕时全部抓取、隐藏→可见切换踢一轮——注意「打开设置」入口对**已打开**面板不触发刷新，但新建面板仍会执行首轮加载〔面板打开踢一脚 + ready 全量刷新〕，因为额度选项卡常驻挂载需要数据）。e2e 通过 `XDG_DATA_HOME` 隔离凭据（保存 Cookie 后的 MiMo 单组刷新会发真实请求，沙箱内失败按 friendly error 断言 `configured` 翻转即可）。
- 备份保留：手动备份永不清理；手动备份可选择仅包含 配置/模板/模型，恢复时可只恢复备份中实际存在的部分范围。`DEFAULT_RETENTION` 里的 pre-* 规则仅为兼容旧版本残留备份。备份创建走暂存目录（`backups/.tmp-*` → rename 发布，失败自动清理，下次创建时 sweep）；恢复时受管文件经 `writeFileAtomic` 写回（ENOSPC/占用不会截断活配置）。Windows 下深层 skills 备份可能触 MAX_PATH（需用户开 LongPathsEnabled）。
